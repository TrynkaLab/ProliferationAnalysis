...
### Step 2: fit peaks
In the simplest case we can run fit_peaks, which estimates peak numbers 
...
the grey dotted line the lower cutoff we set in the gen0.cutoff variable
...
Fix the fit by strictly gating out the CTV- and CTV++ tails prior
to running.
...
As you can see, this produces a nonsensical model as well, as we have a very clear and distinct CTV- sample in this data. If we set peak.max and the the thresholds to 0 we can force only 3 peaks, or we can relax the thresholds so more peaks are considered.
...
if there is a clear peak, the autodetection works well.
...
by the proliferation model. This is useful if the trace has been done on a 
...
bleedover, whereas gating will over-estimate cell counts.
...
peaks <- fit_peaks(y, peak.0.lower.bound=9.9, mode="LS", full.out=T)


dens <- prolif_model_density(peaks$peak.stats$opt_mean,
                     peaks$peak.stats$opt_peak_sd,
                     peaks$peak.stats$opt_summit,
                     peaks$x,
                     log=T)

nll <- prolif_model_nll(peaks$peak.stats$opt_mean,
                     peaks$peak.stats$opt_peak_sd,
                     peaks$peak.stats$opt_summit,
                     peaks$x,
                     log=T)
...
peaks <- fit_peaks(y, peak.0.lower.bound=9.9, mode="MLE", full.out=T, verbose=T)
...
system.time({
fit2 <- fit_peaks(trace=sample.b[sample],
                 peak.0.lower.bound=gen0.cutoff,
                 mode="MLE",
                 opt.trim.left.tail = F,
                 opt.trim.right.tail = T,
...
yy <- prolif_model_density(fit$peak.stats$est_mean,
...
yy <- prolif_model_density(tmp$mu,
                 tmp$sigma,
                 tmp$lambda,
                 fit$x,
                 log=F,
...
yy <- prolif_model_density(fit.mle$peak.stats$opt_mean,
...
yy <- prolif_model(fit.mle$peak.stats$opt_mean,
                 fit.mle$peak.stats$opt_peak_sd,
...
get_prolif_stats(fit$peak.stats[1:(nrow(fit$peak.stats)-1),])
get_prolif_stats(fit.mle$peak.stats[1:(nrow(fit$peak.stats)-1),])
...
peak.x.mode <- find_peak_x_approx_mode(log10(sample.b),
